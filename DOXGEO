...
while True:
    try:
        ...
        if platform.system() == "Windows":
            ...
            enter = pystyle.Colorate.Horizontal(pystyle.Colors.white_to_red, ('Welcome to Anubis, Press "ENTER" to continue! NOW MADE BY GEOHACKER'))
            pystyle.Anime.Fade(
       ...
       ▀███████████ ███   ███ ███    ███ ▀▀███▀▀▀██▄  ███▌ ▀███████████  
         ...
         ███    █▀   ▀█   █▀  ████████▀  ▄█████████▀  █▀    ▄████████▀
                ...
                Welcome to Anubis, Press "ENTER" to continue!
                CRATE THIS DOXX SOFT BY GEOHACKER'''), pystyle.Colors.white_to_red, pystyle.Colorate.Vertical, enter=True)
        def Main():
            if platform.system() == "Windows":
                os.system("cls")
...
[3] ძიება ნიკით
[4] ძიება IP-ით
[5] ძიება მონაცემთა ბაზით
[6] DDoS
[7] პროქსი გენერატორი
[8] ბანვორდ ტექსტი
[9] პაროლების გენერატორი
[10] დოკსის სახელმძღვანელო
[11] სვატის სახელმძღვანელო
[12] ანონიმურობის სახელმძღვანელო
[13] Telegram ანგარიშის წაშლის სახელმძღვანელო
[14] გამოგონილი პიროვნების გენერატორი
[15] Web-crawler
[16] DDOS PRO
[17] გამოგონილი ბარათის გენერატორი
[18] ძიება Mac-მისამართით
[19] პორტ სკანერი
[20] User-agent გენერატორი
[21] ინფორმაცია
[22] დიდების დარბაზი
[23] გამოსვლა                    
...
'''), pystyle.Colors.white_to_red, interval = 0.0005)
        Main()
        while True:
            choice = pystyle.Write.Input("\n\n[?] აირჩიე ფუნქცია -> ", pystyle.Colors.white_to_red, interval = 0.001)
            if choice == "1":
                phone = pystyle.Write.Input("\n[?] დაწერე ტელეფონის ნომერი მაგალითი +99538282928 -> ", pystyle.Colors.white_to_red, interval = 0.005)
                def phoneinfo(phone):
                    try:
                        parsed_phone = phonenumbers.parse(phone, None)
                        if not phonenumbers.is_valid_number(parsed_phone):
                            return pystyle.Write.Print(f"\n[!] ეს ნომერს ერრორი მოვიდა\n", pystyle.Colors.white_to_red, interval=0.005)
                        ...
                        print_phone_info = f"""\n
[+] ტელეფონის ნომერი -> {formatted_number}
[+] ქვეყანა -> {country}
[+] რეგიონო -> {region}
[+] ოპერატორი -> {carrier_info}
[+] მუშაობს -> {is_possible}
[+] ვალიდ -> {is_valid}
[+] დროზონა -> {timezona}
[+] ტელეგრამი -> https://t.me/{phone}
[+] ვათსაპი -> https://wa.me/{phone}
[+] ვიბერი -> https://viber.click/{phone}\n"""
            ...
            if choice == "2":
                ...
                def get_website_info(domain):
                    ...
                    print_string = f"""
[+] დომენი: {domain_info.domain_name}
[+] რეგისტრაციის თარიღი: {domain_info.creation_date}
[+] ვადის გასვლა: {domain_info.expiration_date}
[+] მფლობელი: {domain_info.registrant_name}
[+] ორგანიზაცია: {domain_info.registrant_organization}
[+] მისამართი: {domain_info.registrant_address}
[+] ქალაქი: {domain_info.registrant_city}
[+] შტატი: {domain_info.registrant_state}
[+] საფოსტო კოდი: {domain_info.registrant_postal_code}
[+] ქვეყანა: {domain_info.registrant_country}
[+] IP-მისამართი: {domain_info.name_servers}
        """
                    pystyle.Write.Print(print_string, pystyle.Colors.white_to_red, interval=0.005)
                get_website_info(domain)
            if choice == "3":
                nick = pystyle.Write.Input(f"\n[?] შეიყვანეთ სახელი -> ", pystyle.Colors.white_to_red, interval=0.005)
                urls = [
                ...
                for url in urls:
                    try:
                        ...
                        if response.status_code == 200:
                            pystyle.Write.Print(f"\n{url} - აქუნთი ნაპოვნია ", pystyle.Colors.white_to_red, interval=0.005)
                        elif response.status_code == 404:
                            pystyle.Write.Print(f"\n{url} - აქუნთი არა ნაპოვნი", pystyle.Colors.white_to_red, interval=0.005)
                        else:
                            pystyle.Write.Print(f"\n{url} - ერორი {response.status_code}", pystyle.Colors.white_to_red, interval=0.005)
                    except:
                        pystyle.Write.Print(f"\n{url} - ერორი შემოწმების დროს", pystyle.Colors.white_to_red, interval=0.005)
                print()
            if choice == "4":
                ip = pystyle.Write.Input("\n[?] შეიყვანე იპ ადრესი -> ", pystyle.Colors.white_to_red, interval = 0.005)
                def ip_lookup(ip):
